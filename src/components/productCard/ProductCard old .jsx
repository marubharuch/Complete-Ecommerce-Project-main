        ...
        <section className="text-gray-600 body-font">
            <div className="container px-5 py-8 md:py-16 mx-auto">
                ...
                <div className="flex flex-wrap -m-4">
                    { 
                    product2.filter((obj)=> obj.toLowerCase().includes(searchkey))
                     .filter((obj) => obj.toLowerCase().includes(filterType))
                     .filter((obj) => obj.includes(filterPrice)).slice(0,8).map((item, index) => {
                                ...
                                <div  className="h-full border-2 hover:shadow-gray-100 hover:shadow-2xl transition-shadow duration-300 ease-in-out    border-gray-200 border-opacity-60 rounded-2xl overflow-hidden" style={{ backgroundColor: mode === 'dark' ? 'rgb(46 49 55)' : '', color: mode === 'dark' ? 'white' : '', }} >
                                   {/* <div onClick={()=> window.location.href = `/productinfo/${id}`} className="flex justify-center cursor-pointer" >
                                    ...
                                    <div className="p-5 border-t-2">
                                      ...
                                      }  <h1 className="title-font text-lg font-medium text-gray-900 mb-3" style={{ color: mode === 'dark' ? 'white' : '', }}>{ item.split("#")[0]}={price}
                                        {/* <p className="leading-relaxed mb-3">{item.description.}</p> */}
                                        < div className="leading-relaxed mb-3" style={{ color: mode === 'dark' ? 'white' : '' }}>₹{price}</div></h1>
