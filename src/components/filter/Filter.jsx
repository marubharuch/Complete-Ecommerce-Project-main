...
function Filter() {
    ...
    const { mode, searchkey, setSearchkey,editsearchkey, filterType, setFilterType,
        filterPrice, setFilterPrice, product2,pro2arr } = context
        console.log("pro2arr",filterType)

    return (
        <div>
            <div className=' container mx-auto px-4 mt-5 '>
                <div className="p-5 rounded-lg bg-gray-100 drop-shadow-xl border border-gray-20"
                    style={{
                    ...
                    <div className="relative">
                        ...
                        <input
                            ...
                            //onChange={(e) => editsearchkey(e.target.value)} 
                    ...
                    <div className="flex items-center justify-between mt-4">
                        ...
                        <select value={filterType} onChange={(e)=> setFilterType(e.target.value)} className="px-4 py-3 w-full rounded-md bg-gray-50 border-transparent outline-0 focus:border-gray-500 focus:bg-white focus:ring-0 text-sm" style={{ backgroundColor: mode === 'dark' ? 'rgb(64 66 70)' : '', color: mode === 'dark' ? 'white' : '', }}>
                                ...
                                {pro2arr.map((item, index) => {
                                    return (                                        
                                        // eslint-disable-next-line react/jsx-key
                                        <option value={item.split("#")[2]}>{item}</option>
                                    )
                                })}
                            </select>
                            <button onClick={(e) => {setSearchkey('');setFilterType("")}}  className="px-4 py-2 bg-gray-50hover:bg-gray-200 text-gray-800 text-sm font-medium rounded-md" style={{ color: mode === 'dark' ? 'white' : '' }}>
                            Reset Filter
                        </button>
                    </div>
                    <div>
