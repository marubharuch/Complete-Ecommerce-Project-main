...
function Navbar() {
  ...
  console.log('a',user)
// want to read fro rtdb
const user2 = JSON.parse(localStorage.getItem('user2'));

if(!user2){
 console.log("no User2") 
}else{
  console.log('user2 yes')
}
/*
if(!user2){
const dbRef = ref(getDatabase());
get(child(dbRef, `cart2/`)).then((snapshot) => {
  if (snapshot.exists()) {
    console.log(snapshot.val()); 
    localStorage.setItem('user2', JSON.stringify(snapshot))
  } else {
    ...
    <div className='bg-white sticky top-0 z-50'>
      ...
      <header className="relative bg-white">
        ...
        style={{ backgroundColor: mode === 'dark' ? 'rgb(62 64 66)' : '', color: mode === 'dark' ? 'white' : '', }}>
          Bharuch Jain Business Directory 
        </p>
            ...
            <div className="flex h-16 items-center">
              ...
              <div className="ml-4 flex lg:ml-0">
                <Link to={'/'} className='flex'>
                  <div className="flex ">
                    <h1 className=' text-2xl font-bold text-black  px-2 py-1 rounded' style={{ color: mode === 'dark' ? 'white' : '', }}>Jain's</h1>
                  </div>
                </Link>
              </div>
             ...
             {/* This is desktop menu */}
                <div className="hidden lg:flex lg:flex-1 lg:items-center lg:justify-end lg:space-x-6">
                 ...
                 {user ?  <Link to={'/order'} className="text-sm font-medium text-gray-700 " style={{ color: mode === 'dark' ? 'white' : '', }}>
                  ...
                  {user?.user?.email === 'sanjayshah@gnfc.in' ? 
