...
function Allproducts() {
  ...
  const { mode, product2 ,searchkey, setSearchkey,filterType,setFilterType,
    ...
    <Layout>
      ...
      <section className="text-gray-600 body-font">
            <div className="container px-5 py-8 md:py-16 mx-auto">
                <div>all product without adin</div>
...
{/*( obj.title.toLowerCase().includes(searchkey.split(" ")[0]) || obj.title.toLowerCase().includes(searchkey.split(" ")[1])))
                    .filter((obj)=> obj.title.toLowerCase().includes(searchkey.split(" ")[1])) */}
                <div className="flex flex-wrap -m-4">
                    {product2.filter((obj)=>( obj.title.toLowerCase().includes(searchkey) ))                  
                            ...
                            <div onClick={()=> window.location.href = `/productinfo/${id}`}   key={index} className="p-4 md:w-1/4  drop-shadow-lg " >
                                <div className="h-full border-2 hover:shadow-gray-100 hover:shadow-2xl transition-shadow duration-300 ease-in-out    border-gray-200 border-opacity-60 rounded-2xl overflow-hidden" style={{ backgroundColor: mode === 'dark' ? 'rgb(46 49 55)' : '', color: mode === 'dark' ? 'white' : '', }} >
                                    ...
                                    <div className="p-5 border-t-2">
                                        ...
                                        <h1 className="title-font text-lg font-medium text-gray-900 mb-3" style={{ color: mode === 'dark' ? 'white' : '', }}> {item.split("#")[0]}</h1>
